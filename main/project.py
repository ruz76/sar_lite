...
class Project(object):
    ...
    def createProject(self, index, desc, version):
        ...
        input = KRAJ_DATA_PATH + '/vektor/ZABAGED/line_x/merged_polygons_groupped_fixed.shp'
        output = NEW_PROJECT_PATH +'/pracovni/sektory_group.shp'
        # print("NOW IS THE TIME")
        # print(input)
        # print(output)
        extent = XMIN + "," + XMAX + "," + YMIN + "," + YMAX + " [EPSG:5514]"
        ...
        # output_cur = NEW_PROJECT_PATH +'/pracovni/sektory_group_attributes_1.shp'
        # processing.run("native:addfieldtoattributestable", {'INPUT':output,'FIELD_NAME':'aaa','FIELD_TYPE':0,'FIELD_LENGTH':10,'FIELD_PRECISION':0,'OUTPUT':output_cur})
        output2 = NEW_PROJECT_PATH +'/pracovni/sektory_group_selected.shp'
        processing.run("native:savefeatures", {'INPUT':output,'OUTPUT':output2,'LAYER_NAME':'','DATASOURCE_OPTIONS':'','LAYER_OPTIONS':''})
