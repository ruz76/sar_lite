...
class Sectors(object):
    ...
    def getDistancePosition(self, percentage):
        if percentage <= 10:
            return 0
        ...
        if percentage > 80:
            return 8
    def getDistance(self, percentage, DATAPATH):
        with open(DATAPATH + '/pracovni/project.json') as f:
            data = json.load(f)
            persontype = data["persontype"]
            with open(self.pluginPath + "/grass/distances.txt") as d:
                lines = d.readlines()
                line = lines[persontype - 1]
                items = line.split(',')
                return items[self.getDistancePosition(percentage)]
    ...
    def getSectors(self, min, max):
        ...
        # TODO set distance based on max an person type from project
        distance = self.getDistance(max, DATAPATH)
        processing.run("native:extractwithindistance", {'INPUT':DATAPATH + '/pracovni/sektory_group.shp','REFERENCE':DATAPATH + '/pracovni/mista.shp','DISTANCE':distance,'OUTPUT':DATAPATH + '/pracovni/sektory_group_selected.shp'})
        # TODO export CSV ?
    ...
    def reportExportSectors(self, openReport, exportPDF):
        ...
        # Reads units report
        try:
            if os.path.exists(DATAPATH + '/pracovni/report.html.units'):
                report_units = io.open(DATAPATH + '/pracovni/report.html.units', encoding='utf-8', mode='r').read()
            else:
                report_units = 'NOT SUPPORTED IN LITE VERSION'
        ...
        for feature in features:
            ...
            # Reads sector report
            if os.path.exists(DATAPATH + '/pracovni/report.html.' + str(i)):
                report = io.open(DATAPATH + '/pracovni/report.html.' + str(i), encoding='utf-8', mode='r').read()
            else:
                report = 'NOT SUPPORTED IN LITE VERSION'
            f.write(report)
